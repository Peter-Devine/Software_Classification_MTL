...
for dataset in ${datasets[@]}; do
	for random_state in ${random_states[@]}; do
					...
					echo "Now running dataset $dataset with zero_shot_datasets $ in random state $random_state"

	        python software_feedback_cls.py --dataset_list=$dataset --random_state=$random_state --zero_shot_label=bug --zero_shot_dataset_list=jha_2017,scalabrino_2017,maalej_2016,williams_2017,di_sorbo_2017,guzman_2015,tizard_2019 --output_text --do_classical --num_epochs=60 --num_fine_tuning_epochs=60 --early_stopping_patience=10 --batch_size_train=32 --batch_size_eval=32 --max_length=128 --neptune_username=$1
	done
done
